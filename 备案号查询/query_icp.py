...
def query_base(domain):
    while True:
        try:
            info = domain.replace(" ", "").replace("https://www.", "").replace("http://www.",
                                                                               "").replace(
                "http://", "")
    ...
    while err_num < 3:
        try:
            cookie = requests.utils.dict_from_cookiejar(
                requests.get('https://beian.miit.gov.cn/', headers=cookie_headers).cookies)[
                '__jsluid_s']
            return cookie
        except:
            err_num += 1
...
if __name__ == '__main__':
    domain_list = ['baidu.com', 'aliyun.com', 'taobao.com', 'tencent.com', 'qq.com']
    ...
    with open(r'result.txt', 'a+') as file:
        for i in domain_list:
            if i not in all_result:
                time.sleep(3)
                icp_number = main(i)[0][2]
                result = main2(icp_number)
                for info in result:
                    all_result.append(info[1])
                    file.write(info[1] + '\n')
